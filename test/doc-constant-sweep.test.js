...
const PINS = [
  ...
  },
  {
    ...
    checks: [
      { file: 'orchestrator/config.js', contains: "boundedPositiveIntFromEnv('SPO_BENCH_IDLE_WAIT_MAX_POLLS', 180, BENCH_IDLE_WAIT_MAX_POLLS_CEILING)," },
  ...
  },
  {
    ...
    checks: [
      { file: 'orchestrator/config.js', contains: "boundedPositiveIntFromEnv('SPO_CI_CHECKS_MAX_POLLS', 30, CI_CHECKS_MAX_POLLS_CEILING)," },
...
const EXPECTED_CITATIONS = [
  ...
  "doc/bench-audit-2026-09-02.md :: doc/state-machine-spec.md:623", // re-pinned from :157, then :166, then :207 (card #212 C1), then :270 (card #212 C2), then :315, then :318 -- card #212's fix pass (F1/F2/F3/F6, 45 net lines) landed above the step table, a true pure shift; content byte-identical at :315 (verified: the FINISH row's own "Action B1.4: FINISH now actually keeps the promise this row always made -- fast-forward the main product checkout" opens it). Re-pinned again in card #212's C4/C5 build (Lot, 2026-09-14): :318 -> :387, a pure +69-line shift when doc/state-machine-spec.md gained the `continue` verb's own "Resume at CHECK" subsection above the step table; content byte-identical at :387, verified by re-reading the target line. Closing testimony corrected for card #239 A9 (the value had moved :387 -> :389 without this sentence being told, which the Rule B check in this file found): content byte-identical at :389, verified by re-reading the target line. Card #251 (2026-09-24): :389 -> :445, the "Machine resume after a pool-wait" subsection landed above the step table, a pure shift; content byte-identical at :445, verified by re-reading the target line. Card #255 (2026-09-25): :445 -> :470, same +25-line shift; content byte-identical at :470 (the FINISH row), verified by re-reading the target line. Card #279 (2026-09-25): :470 -> :514, the option-B rewrite of the #212 C4 decision note, the resume-at-IMPLEMENT validation/journal paragraphs and prepareResume's step 6/9 exceptions all landed above the step table, a pure +44-line shift, then :514 -> :518 in the same card's doc round (the resume-at-IMPLEMENT journal paragraph gained the prepareResume-deadline exception, +4 lines); content byte-identical at :518 (the FINISH row), verified by re-reading the target line. Card #281 (2026-09-25): :518 -> :567, the closed-gap paragraph that replaces #279's accepted gap, the step 6/9 rewrites keyed on the machine re-enqueue, and the refusal-after-a-kept-tree exception all landed above the step table, a pure +49-line shift; content byte-identical at :567 (the FINISH row), verified by re-reading the target line. Then :567 -> :610 in card #281's verifier fix pass (the F1 pr-read-failed paragraph, the F4 residual and the F5 "what the next continue runs on" paragraph, +43 lines above the step table, a pure shift); content byte-identical at :610 (the FINISH row), verified by re-reading the target line. Then :610 -> :623 in the card's final doc round (the F5 push paragraph's fast-forward qualifier and never-force warning, the F4 residual's qualifier and the F1 continue-carried clause, +13 lines above the step table, a pure shift); content byte-identical at :623 (the FINISH row), verified by re-reading the target line.
  ...
  "orchestrator/README.md :: config.js:1185", // PORT (card #167 onto main, 2026-09-23): :1069 -> :1072, card #167 widened the `workers` K-clamp comment (+2) and the claudeAccountsDir comment (+1), both above productRepo, +3 net lines; content byte-identical at :1072, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23, chantier/sdk-transport + main #241): branch had :968 (action A2/A9 history), main independently re-pinned to :1000 (card #224's own MERGE_STEP_DEADLINE_MS block above productRepo). Both real, both merged; content byte-identical at :1072 (`productRepo: process.env.SPO_PRODUCT_REPO || path.join(os.homedir(), 'SPO-WebClient'),`), verified by re-reading the target line. PR #249 does not touch config.js, so this merge leaves it unchanged. Then RE-PIN (card #225, 2026-09-24): :1072 -> :1127, card #225 hoisted MAX_TIMER_DELAY_MS and added the CI_CHECKS and BENCH_IDLE_WAIT poll-count ceilings (with their comments) above productRepo, +55 net lines; content byte-identical at :1127 (`productRepo: process.env.SPO_PRODUCT_REPO || path.join(os.homedir(), 'SPO-WebClient'),`), verified by re-reading the target line. Then RE-PIN (card #246, 2026-09-24): :1127 -> :1138, card #246 rewrote config.js's BENCH_IDLE_WAIT_MAX_MS justification comment above productRepo (the re-derived bench-queue-wait constants, +11 net lines); content byte-identical at :1138, verified by re-reading the target line. Then RE-PIN (card #259, 2026-09-25): :1138 -> :1174, card #259 added productRepoStepDeadlinesMs (the one WORKTREE/FINISH derivation daemon.js's --workers recompute now shares), moved WORKTREE/FINISH out of the stepDeadlineMsByState literal into it, and widened boundedPositiveIntFromEnv's header above productRepo, +36 net lines; content byte-identical at :1174, verified by re-reading the target line. Then RE-PIN (card #267, 2026-09-25): :1174 -> :1185, card #267 split boundedPositiveIntFromEnv's rule out into an exported parseBoundedPositiveInt and added the shared PER_CYCLE_LIMIT_CEILING constant (with their comments) above productRepo, +11 net lines; content byte-identical at :1185, verified by re-reading the target line.
  ...
  "orchestrator/README.md :: doc/state-machine-spec.md:616", // MERGE (2026-09-23): branch had :382 (A9's own closing-testimony correction), main independently re-pinned to :380 (its own history stops one hop earlier, before A9's correction). Content byte-identical at :382 (the CHECK row, "| CHECK | script | invariant substring check first..."), verified by re-reading the target line -- the branch's number, confirmed rather than assumed. #249 does not touch doc/state-machine-spec.md's step table. Card #251 (2026-09-24): :382 -> :438, a pure shift from the "Machine resume after a pool-wait" subsection above the step table; content byte-identical at :438, verified by re-reading the target line. Card #255 (2026-09-25): :438 -> :463, the option-A decision note in "Machine resume after a pool-wait" and the counters clause above the step table, a pure +25-line shift; content byte-identical at :463, verified by re-reading the target line. Card #279 (2026-09-25): :463 -> :507, the option-B rewrite of that decision note plus the resume-at-IMPLEMENT paragraphs above the step table, a pure +44-line shift, then :507 -> :511 in the same card's doc round (+4 lines, the prepareResume-deadline exception); content byte-identical at :511 (the CHECK row), verified by re-reading the target line. Card #281 (2026-09-25): :511 -> :560, the closed-gap paragraph, the step 6/9 rewrites and the refusal-after-a-kept-tree exception above the step table, a pure +49-line shift; content byte-identical at :560 (the CHECK row), verified by re-reading the target line. Then :560 -> :603 in card #281's verifier fix pass (the F1/F4/F5 paragraphs above the step table, +43 lines, a pure shift); content byte-identical at :603 (the CHECK row), verified by re-reading the target line. Then :603 -> :616 in the card's final doc round (+13 lines above the step table, a pure shift); content byte-identical at :616 (the CHECK row), verified by re-reading the target line.
  "orchestrator/README.md :: intake.js:996-998", // PORT (card #167 onto main, 2026-09-23): :969-971 -> :989-991, card #167's INTAKE_MODELS import comment and its rotation-helper header comments land above triageBugReport's header, +20 net lines; content byte-identical at :989-991, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23): same fact and same correction as the doc/state-machine-spec.md :: intake.js entry above (A2's +5, card #240's +10, then #249's own +1-line OPUS_5_5 require, additive); content byte-identical at :989-991, verified by re-reading the target lines. Re-pinned in card SPO-Pipeline#250 (2026-09-24): :989-991 -> :996-998, callIntakeStepWithRotation's markLimit call gained its limitScope/rateLimitType arguments and comment above this point, a pure +7-line shift; content byte-identical at :996-998, verified by re-reading the target lines.
  ...
  "orchestrator/auto-triage.js :: state-machine.js:4182", // PORT (card #167 onto main, 2026-09-23): :3591 -> :3620, card #167's resolveCallModel import comment and callLlmStep's stepModel block land above this point, +29 net lines; content byte-identical at :3620, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23): main's own history (card #231, then card #226's fix pass, +16 then +139) is what applies here -- the branch's :3436 predates both, since neither #231 nor #226 exist on chantier/sdk-transport. Content byte-identical at :3620 (the "Verification fix: appendDaemonEvent itself does an mkdirSync + appendFileSync -- on the" comment line), verified by re-reading the target line. Card #251 (2026-09-24): :3620 -> :3790, the machine-resume helpers (countersForResume/restoreResumeCounters/poolWaitResume/restartRefusedMachineResume) landed above this point, a pure shift; content byte-identical at :3790, verified by re-reading the target line. Card SPO-Pipeline#250 (2026-09-24): :3790 -> :3797, callLlmStep's markLimit call gained its limitScope/rateLimitType arguments and comment above this point, a pure +7-line shift; content byte-identical at :3797, verified by re-reading the target line. SPO-Pipeline#166 (2026-09-24): :3797 -> :3918, the judge quota fallback (callLlmStep header and model switch, quotaFallbackMark) landed above this point, a pure +121-line shift; content byte-identical at :3918, verified by re-reading the target line. SPO-Pipeline#166 action 2 (2026-09-25, model-aware K-clamp): :3918 -> :3946, takeNextTask's `admit` header comment and its take/skip/hold admission check landed above this point, a pure +28-line shift; content byte-identical at :3946, verified by re-reading the target line. SPO-Pipeline#277 (2026-09-25, with its verifier finding F1, the driver's 529 refinement and the re-verification fixes): :3946 -> :3982, callLlmStep's header gained the #277 rule (rotate on Fable first; fall back only when no account has Fable quota -- a 529 not counting -- onto one with Opus 5.5), its decision-4 and `model-fallback` cause notes, both triggers now ask accounts.quotaFallbackServable, and the rotation loop gained its `deps.runLlm` test-seam comment, all above this point, a pure +36-line shift; content byte-identical at :3982, verified by re-reading the target line. SPO-Pipeline#279 (2026-09-25, option B): :3982 -> :4019, carriedResume's startState rule, resumeValidationError's RESUME_START_STATES, handleImplement's resume note and runTask's resume-at-IMPLEMENT path all landed above this point, a pure +37-line shift, then :4019 -> :4023 in the same card's doc round (POOL_WAIT_RESUME_STATES' `continue` exception, runTask's deadline note and takeNextTask's IMPLEMENT-row note, +4 lines); content byte-identical at :4023, verified by re-reading the target line. SPO-Pipeline#281 (2026-09-25): :4023 -> :4079, buildCtx's resumeKeptDirtyTree/wipReattachBranch fields, isMachineReEnqueueResume, finalizePark's re-attach and runTask's keepInFlightWork/refusal-after-a-kept-tree catch all landed above this point, a pure +56-line shift; content byte-identical at :4079, verified by re-reading the target line. Then :4079 -> :4101 in the same card's verifier fix pass (the F1 pr-read-failed probe in runTask's refusal catch and its comments, +22 lines, a pure shift); content byte-identical at :4101, verified by re-reading the target line. SPO-Pipeline card 51 (2026-09-25): :4101 -> :4182, flattenGroupedFiles/implementStopReason and the implement-stopped park landed above it, a pure +81-line shift; content byte-identical at :4182, verified by re-reading the target line.
  "orchestrator/bench-queue-wait.js :: SPO-WebClient/src/e2e/bench/job.ts:358", // RE-CITED (card #246, 2026-09-24): :361 -> :358, and not a drift -- the CLAIM changed. The prose used to cite purgeDone's mtime-gated `fs.rmSync(file, { force: true })` (:361) as deleting every old report, false since SPO-WebClient 215e1083 (B4.2, 2026-09-03); it now cites the filter that makes it false, `if (!name.endsWith('.log')) continue;`, at :358 (purgeDone's own declaration is :356), verified by reading SPO-WebClient/src/e2e/bench/job.ts at origin/main 4fa45a35 directly. Before that: re-pinned from :325 when job.ts drifted independently of this repo.
  ...
  "orchestrator/invariants.js :: doc/state-machine-spec.md:616", // MERGE (2026-09-23): same fact and same correction as the orchestrator/README.md :: doc/state-machine-spec.md entry above -- :382 is what the merged doc holds (A9's own closing-testimony correction), not main's independent :380. Card #251: :382 -> :438, same shift as the orchestrator/README.md entry above. Card #255: :438 -> :463, same shift as the orchestrator/README.md entry above. Card #279: :463 -> :507, then :511 in its doc round, same shifts as the orchestrator/README.md entry above. Card #281: :511 -> :560, same +49-line shift as the orchestrator/README.md entry above; content byte-identical at :560, verified by re-reading the target line. Its verifier fix pass: :560 -> :603, same +43-line shift as the orchestrator/README.md entry above. Its final doc round: :603 -> :616, same +13-line shift as that entry; content byte-identical at :616, verified by re-reading the target line.
  "orchestrator/invariants.js :: relative/path/to/file.ts:123",
  "orchestrator/journal.js :: auto-pull.js:58-66",
  "orchestrator/orphan-scan.js :: auto-pull.js:58-66",
  "orchestrator/orphan-scan.js :: daemon.js:944", // re-pinned from :912, then :942, then :955 (action A2, card #239) -- see this file's own prior history at each hop. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :955 -> :961, the SAME +6-line shift as dispatcher.js's daemon.js:650 -> :656 re-pin above; content byte-identical at :963 (`const recoveredOrphans = await orphanScan(queueDir, journalRoot, config);`), verified by re-reading the target line. PR #249 does not touch daemon.js, so this second merge leaves it unchanged. Then RE-PIN (card #259, 2026-09-25): :963 -> :944, card #259 replaced daemon.js's own restated WORKTREE/FINISH formulas in main()'s stepDeadlineMsByState recompute with one call to config.js's productRepoStepDeadlinesMs, -19 net lines above this call (the product-repo-hold.js require it dropped became a one-line comment, so every citation above the recompute keeps its line); content byte-identical at :944, verified by re-reading the target line.
  ...
  "orchestrator/park-loop.js :: intake.js:996-998", // PORT (card #167 onto main, 2026-09-23): :969-971 -> :989-991, card #167's INTAKE_MODELS import comment and its rotation-helper header comments land above triageBugReport's header, +20 net lines; content byte-identical at :989-991, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23): branch's A2 (+5) and main's card #240 (+10) land additively at :968-970, then PR #249's own +1-line OPUS_5_5 require shifts it one further to :969-971 -- same fact and same correction as the doc/state-machine-spec.md :: intake.js entry above; content byte-identical at :989-991, verified by re-reading the target lines. Re-pinned in card SPO-Pipeline#250 (2026-09-24): :989-991 -> :996-998, callIntakeStepWithRotation's markLimit call gained its limitScope/rateLimitType arguments and comment above this point, a pure +7-line shift; content byte-identical at :996-998, verified by re-reading the target lines.
  "orchestrator/state-machine.js :: auto-pull.js:58-66",
  "orchestrator/state-machine.js :: auto-pull.js:58-66",
  "orchestrator/state-machine.js :: orchestrator/steps/llm.js:1273", // PORT (card #167 onto main, 2026-09-23): :1116 -> :1149, card #167's resolveCallModel and its header land directly above runLlm, +33 net lines; content byte-identical at :1149, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23): branch's own history (through A9's CI fix pass) reaches :1113; main's card #240 (its own +14-line buildArgv --disallowedTools addition, since main never had A9's chantier) reaches :1063. Neither number is right for the merged tree -- both real edits land. Content byte-identical at :1149 (still `const contract = resolveStepContract(stepName, ctx.task || {});`), verified by re-reading the target line. Re-pinned in card #254's review fix pass (2026-09-24): :1149 -> :1173, a pure +24-line shift from invokeClaudeReal's deadline-branch limitKind drop, its dated correction and the signal-after-result decision comment, all above runLlm; content byte-identical at :1173, verified by re-reading the target line. Re-pinned in card SPO-Pipeline#250 (2026-09-24): :1173 -> :1257, limitScopeFor with its Sets, constants and header (and the verifier fix pass's typed cross-check) land above runLlm, plus the deadline branch's limitScope drop, a pure +84-line shift; content byte-identical at :1257, verified by re-reading the target line. SPO-Pipeline#166 (2026-09-24): :1257 -> :1272, resolveQuotaFallbackModel and the STEP_CONTRACTS import land above runLlm, a pure +15-line shift; content byte-identical at :1272, verified by re-reading the target line. SPO-Pipeline#277 (2026-09-25): :1272 -> :1273, resolveQuotaFallbackModel's header gained one line (the #277 trigger), a pure +1-line shift; content byte-identical at :1273, verified by re-reading the target line.
  ...
  "orchestrator/state-machine.js :: step-contracts.js:1329", // PORT (card #167 onto main, 2026-09-23): :1175 -> :1203, card #167's INTAKE_MODELS block (after OPUS_5_5) and its DIAGNOSE AVAILABILITY erratum land above this point, +28 net lines; content byte-identical at :1203, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23): branch's own history (through A9's closing-testimony correction) reached :1146; main's card #231 (+25 lines) then PR #249's own Opus-5.5 refactor (the OPUS_5_5 constant, the IMPLEMENT effort-map note, and escalationSignalFires replacing shouldEscalate's inline body, all above this point) reached :1102 on main alone. Neither number is right for the merged tree -- card #240's `./bash-policy`/disallowedTools addition (already in branch's :1146), plus main's #231 and #249 additions, all land; content byte-identical at :1203 (`if (task.touchesRdoMembers === true) return true; // source 3: intake's guess, undeclared plan`), verified by re-reading the target line rather than summed from either side's own arithmetic. SPO-Pipeline#166 (2026-09-24): :1203 -> :1239, the quotaFallbackModel preamble and the two contract fields landed above it, a pure +36-line shift; content byte-identical at :1239, verified by re-reading the target line. SPO-Pipeline#277 (2026-09-25, with its verifier finding F1 and the re-verification fixes): :1239 -> :1244, the quotaFallbackModel preamble gained three lines and the two judge contracts' fallback comments one each (the #277 rule), a pure +5-line shift; content byte-identical at :1244, verified by re-reading the target line. SPO-Pipeline card 48 (2026-09-25): :1244 -> :1249, IMPLEMENT's `optional: ['commit_subject']` and its four-line comment landed above it, a pure +5-line shift; content byte-identical at :1249, verified by re-reading the target line. SPO-Pipeline#221 (2026-09-26): :1249 -> :1329, the B-scoped schema-only notes (module header, checkOutputTypes' SCHEMA-ONLY comment) and the four contracts' new `types`/`schemaOnly` entries landed above it, a pure +80-line shift; content byte-identical at :1329, verified by re-reading the target line.
  "orchestrator/steps/llm.js :: intake.js:996-998", // PORT (card #167 onto main, 2026-09-23): :969-971 -> :989-991, card #167's INTAKE_MODELS import comment and its rotation-helper header comments land above triageBugReport's header, +20 net lines; content byte-identical at :989-991, verified by re-reading the target line(s) in the merged tree. Before that: MERGE (2026-09-23): same fact and same correction as the doc/state-machine-spec.md :: intake.js entry above; content byte-identical at :989-991, verified by re-reading the target lines. Re-pinned in card SPO-Pipeline#250 (2026-09-24): :989-991 -> :996-998, callIntakeStepWithRotation's markLimit call gained its limitScope/rateLimitType arguments and comment above this point, a pure +7-line shift; content byte-identical at :996-998, verified by re-reading the target lines.
  ...
  "orchestrator/steps/sdk-call.js :: llm.js:1220", // PORT (card #167 onto main, 2026-09-23): :1063 -> :1096, card #167's resolveCallModel and its header land above runLlm, +33 net lines; content byte-identical at :1096 (`jsonSchema: override.jsonSchema,`), verified by re-reading the target line in the merged tree. Before that: MERGE (2026-09-23): branch's own history (through the CI fix pass) reached :1060; main's card #240 (disallowedTools additions to runLlm's legacy override branch and to buildContractOptions, auto-merged cleanly into this file, unrelated to the sdk-transport conflict) added lines above this target too. Content byte-identical at :1096 (`jsonSchema: override.jsonSchema,`), verified by re-reading the target line. Re-pinned in card #254's review fix pass (2026-09-24): :1096 -> :1120, the same pure +24-line shift in invokeClaudeReal as the state-machine.js entry above; content byte-identical at :1120 (`jsonSchema: override.jsonSchema,`), verified by re-reading the target line. Card SPO-Pipeline#250 (2026-09-24): :1120 -> :1204, the same pure +84-line shift as the state-machine.js entry above; content byte-identical at :1204 (`jsonSchema: override.jsonSchema,`), verified by re-reading the target line. SPO-Pipeline#166 (2026-09-24): :1204 -> :1219, resolveQuotaFallbackModel landed above it, a pure +15-line shift; content byte-identical at :1219, verified by re-reading the target line. SPO-Pipeline#277 (2026-09-25): :1219 -> :1220, resolveQuotaFallbackModel's header gained one line, a pure +1-line shift; content byte-identical at :1220, verified by re-reading the target line.
  "orchestrator/steps/sdk-call.js :: llm.js:1220", // same target, cited a second time at the jsonSchema-parsing block's own comment further down this same file -- occurrences count separately here, same convention as the orchestrator/README.md:252 pair below. MERGE (2026-09-23) and PORT (card #167): same corrections as the row above, and card #254's review fix pass (2026-09-24, :1096 -> :1120) too. Card SPO-Pipeline#250 (2026-09-24, :1120 -> :1204) too. SPO-Pipeline#166 (:1204 -> :1219) and SPO-Pipeline#277 (2026-09-25, :1219 -> :1220) too.
...
const ANCHOR_BLUNT_CITATIONS = {
  // README.md: "`doc/state-machine-spec.md:616` has always promised CHECK runs an invariant
  ...
  // row today. Then card #251 (:380 -> :438), card #255 (:438 -> :463), card #279 (:463 ->
  // :507 -> :511) and card #281 (:511 -> :560 -> :603 -> :616, the second in its verifier fix pass, the third in its final doc round), the same pure shifts.
  'orchestrator/README.md :: doc/state-machine-spec.md:616':
    ...
    'Citation confirmed correct by hand: 616 is the CHECK row.',
...
const EXPECTED_DISCRIMINATING_CITATIONS = [
  ...
  'doc/board-audit.md :: config.js:1343',
  'doc/board-audit.md :: orchestrator/steps/scripted.js:1472',
  'doc/board-audit.md :: report-intake.js:29',
  'doc/state-machine-spec.md :: bin/spo:1257',
  'orchestrator/README.md :: config.js:1185',
  'orchestrator/README.md :: lock.js:352',
  'orchestrator/README.md :: lock.js:386',
  'orchestrator/auto-triage.js :: park-loop.js:1569',
  'orchestrator/auto-triage.js :: remote-report-pull.js:193',
  'orchestrator/auto-triage.js :: state-machine.js:4182', // re-pinned from :3574, see EXPECTED_CITATIONS's own entry above for the corrected shift. Card SPO-Pipeline#250: then :3790 -> :3797 (pure shift). SPO-Pipeline#166: then :3797 -> :3918 (pure shift). SPO-Pipeline#166 action 2: then :3918 -> :3946 (pure shift). SPO-Pipeline#277: then :3946 -> :3982 (pure shift). SPO-Pipeline#279: then :3982 -> :4019 -> :4023 (pure shifts). SPO-Pipeline#281: then :4023 -> :4079 -> :4101 (pure shifts). SPO-Pipeline card 51: then :4101 -> :4182 (pure shift).
  'orchestrator/bench-queue-wait.js :: SPO-WebClient/src/e2e/bench/job.ts:358', // re-cited from :361 by card #246 (the claim changed), see EXPECTED_CITATIONS's own entry above.
  ...
  'orchestrator/invariants.js :: doc/state-machine-spec.md:616',
  'orchestrator/orphan-scan.js :: daemon.js:944',
  'orchestrator/park-loop.js :: doc/remediation-plan-2026-08.md:221',
  'orchestrator/state-machine.js :: orchestrator/steps/llm.js:1273', // card #254 review fix pass: re-pinned from :1149, see EXPECTED_CITATIONS's own entry above. Card SPO-Pipeline#250: then :1173 -> :1257 (pure shift). SPO-Pipeline#166: then :1257 -> :1272 (pure shift). SPO-Pipeline#277: then :1272 -> :1273 (pure shift).
  'orchestrator/state-machine.js :: run.ts:63',
  'orchestrator/state-machine.js :: step-contracts.js:1329', // SPO-Pipeline#221: re-pinned from :1249 (pure +80-line shift; card 48 before it moved :1244 -> :1249), see EXPECTED_CITATIONS's own entry above.
  'orchestrator/steps/scripted.js :: run.ts:63',
  'orchestrator/steps/scripted.js :: worker.ts:1636',
  'orchestrator/steps/sdk-call.js :: daemon.js:111',
  'orchestrator/steps/sdk-call.js :: llm.js:1220', // card #254 review fix pass: re-pinned from :1096, see EXPECTED_CITATIONS's own entry above. Card SPO-Pipeline#250: then :1120 -> :1204 (pure shift). SPO-Pipeline#166: then :1204 -> :1219 (pure shift). SPO-Pipeline#277: then :1219 -> :1220 (pure shift).
  'orchestrator/steps/sdk-call.js :: llm.js:1220',
  'prompts/README.md :: plan.md:103',
];
...
test('MUTATION PROOF, corpus-wide: every single-line citation the anchor check accepts stops anchoring when its line is shifted by one', () => {
  ...
  // classifies UNANCHORABLE instead (`orchestrator/park-loop.js :: intake.js:996-998` -- this
...
const EXPECTED_BENCH_PIN_KEYS = [
  ...
  "doc/bench-audit-2026-09-02.md :: doc/state-machine-spec.md:623 @ HEAD",
  ...
  "doc/bench-plan-derived-2026-09-02.md :: doc/state-machine-spec.md:623 @ HEAD",
...
const EXPECTED_LIVE_PIN_KEYS = [
  ...
  "doc/state-machine-spec.md :: intake.js:996-998 @ HEAD",
  ...
  "orchestrator/README.md :: doc/state-machine-spec.md:616 @ HEAD",
  "orchestrator/README.md :: intake.js:996-998 @ HEAD",
...
const DATED_DOC_HEAD_EXCEPTIONS = new Set([
  ...
  "doc/bench-audit-2026-09-02.md :: doc/state-machine-spec.md:623", // the FINISH row -- hand-maintained as true today, not dated record (BENCH_PINS header); re-pinned from :166 by main's own card #212 merge, a true pure shift
  "doc/bench-plan-derived-2026-09-02.md :: doc/state-machine-spec.md:623", // same fact, same reason, cited from the sibling doc
]);
...
test('MUTATION PROOF: reverting bin/spo:1243 back to bin/spo:1129 (the drift this check caught again) makes it red, on the real files', () => {
  ...
  // times.
  //
  // NINETEENTH catch, card #269 (2026-09-25): `cmdStatus` gained the `dispatcher: HELD` branch and
  // the model-naming idle wording, and the file's own `spo status` inventory two lines for HELD,
  // all above `collectAll(sources)` -- :1284 -> :1299. The canary stays `:1129`: re-checked
  // empirically (this test, green on the re-pinned file): the shift is +15 in all -- 4 lines of
  // header inventory near the top of bin/spo plus 11 in `cmdStatus` -- and only the 4 header lines
...
test('every EXPECTED_CITATIONS pin\'s closing "byte-identical at" clause names the pin\'s own line (Rule B)', () => {
  ...
  // clause with their re-pin -- "doc/bench-plan-derived-2026-09-02.md :: doc/state-machine-spec.md:623"
  // and "orchestrator/invariants.js :: doc/state-machine-spec.md:616" -- so Rule B now checks them too.
...
test('Rule B is live against the real registry: re-introducing a stale closing clause turns it red', () => {
  ...
  const target = entries.find((e) => e.key === 'orchestrator/README.md :: config.js:1185');
  ...
  const mutated = entries.map((e) =>
    e === target ? { key: e.key, prose: e.prose.replace(/content byte-identical at :1185/g, 'content byte-identical at :968') } : e
  );
