...
//      citation established the file -- `` `account-lease.js:167` -> `lock.js:352` ... -> `:386`
...
const EXPECTED_CITATIONS = [
  ...
  "doc/board-audit.md :: config.js:1104", // re-pinned from :900, then :965, then :1003, then :1035, then :1097 (action A2, card #239) -- see this file's own prior history at each hop. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :1097 -> :1102, a pure +5-line shift when F3's own config.js edits (correcting the LLM_STEP_DEADLINE_ENTRIES/MAX_LEASE_AGE_MS-table comments from future to past tense) landed above reportIntakeColumn; content byte-identical at :1102, verified by re-reading the target line. Closing testimony corrected for card #239 A9 (the value had moved :1102 -> :1104 without this sentence being told, which the Rule B check in this file found): content byte-identical at :1104, verified by re-reading the target line.
  "doc/board-audit.md :: orchestrator/steps/scripted.js:1396", // re-pinned from :1382, then :1389 (card #212) -- see this file's own prior history. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :1389 -> :1394, a pure +5-line shift when F3's own scripted.js header edit (the dry-run/--real prose, `query()` options not argv) landed above realWorktree; content byte-identical at :1394. Re-pinned once more in the SAME fix pass's second sweep round (team-lead cross-check): :1394 -> :1396, a pure +2-line shift when the round's own `runSync`'s `deps.spawnSync` comment fix landed above realWorktree too; content byte-identical at :1396, verified by re-reading the target line.
  "doc/board-audit.md :: report-intake.js:29",
  "doc/state-machine-spec.md :: bin/spo:1242", // re-pinned in card #214 (Lot 9, 2026-09-13): :1202 -> :1232, a pure +30-line shift when that action's `cmdTokens` gained the opt-in `--usage-delta` section (see this file's own EXPECTED_CITATIONS entry for `bin/spo:1284`, the `collectAll` pin shifted by the same edit) landed above `cmdDashboard` in the same file. Re-pinned again in card #219 (2026-09-14): :1232 -> :1235, a pure +3-line shift when that card's bin/spo cmdStatus injection (monotonicNowMs require plus two option lines) landed above `cmdDashboard`. Re-pinned a third time in card #219's OWN fix pass, same day: :1235 -> :1242, a pure +7-line shift when the fix pass expanded that same injected-deps comment (naming `processStartUptimeMs`/`monotonicNowMs()` explicitly) above `cmdDashboard` in the same file; content byte-identical (`function cmdDashboard(opts) {`) at :1242, verified by re-reading the target line.
  "doc/state-machine-spec.md :: dispatcher.js:643-656", // re-pinned from :634-648 -- action 11.1 (#206) found :634 is a blank line (unable to discriminate any drift against it, and blank text pins do not narrow `movedTo` either); :635 is the "if (childrenSignalled && outcome === 'crashed') {" line this citation actually names, verified by reading. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :635-648 -> :643-656, a pure +8-line shift when F3's own dispatcher.js header edit landed above this block; content byte-identical, verified by re-reading.
  "doc/state-machine-spec.md :: intake.js:958-960", // re-pinned from :797-799, then :854-856 by issue #196's own action (+57 lines), then :869-871, then :906-908 by the Priority-as-a-field action (+37 lines); issue #198's resolveLabelArgs extraction (2026-09-12) added a net 30 lines to intake.js above triageBugReport's own header comment (the new shared label-inventory helper used by fileCard and amendCard, netted against fileCard's shrunk header/inlined-label-code and amendCard's grown header). A TRUE pure shift: content byte-identical at :936-938, diffed line by line against the pre-action file. Re-pinned again in card #214 (Lot 9, 2026-09-13): :936-938 -> :938-940, a pure +2-line shift when that action's journalIntakeLlmCall fix (removing `numTurns: raw.numTurns,` and adding a 3-line replacement comment explaining why) landed above this point in the same file; content byte-identical at :938-940, verified by re-reading the target lines. Re-pinned again in card #218 (Lot 12, 2026-09-14): :938-940 -> :953-955, a net +15-line shift when that action's resolveLabelArgs fix (issue #218: replacing the binary `flag === '--label' ? ... : ...` ternaries with the frozen, throwing LABEL_ARGS_BY_FLAG lookup, plus the corrected header-comment sentence) landed above this point in the same file; content byte-identical at :953-955, verified by re-reading the target lines. Re-pinned again for action A2 (card #239, 2026-09-17): :953-955 -> :958-960, a pure +5-line shift when that action's own comment above intake.js's INTAKE_DEADLINE_MS block landed above this point in the same file; content byte-identical at :958-960, verified by re-reading the target lines.
  "orchestrator/README.md :: .claude/hooks/context-router.sh:117",
  ...
  "orchestrator/README.md :: account-lease.js:167", // re-pinned from :156 -- action A2 (card #239, 2026-09-17) added ~11 net lines to the "why 2x" and "roughly 7.6x the longest full two-attempt step" paragraphs above tryAcquireLease in the same file, a true pure shift; content byte-identical at :167, verified by re-reading the target line.
  "orchestrator/README.md :: config.js:968", // re-pinned from :773, then :829, then :867, then :899, then :961 (action A2, card #239) -- see this file's own prior history at each hop. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :961 -> :966, the SAME +5-line shift as config.js:1097 -> :1102 above; content byte-identical at :966, verified by re-reading the target line. Closing testimony corrected for card #239 A9 (the value had moved :966 -> :968 without this sentence being told, which the Rule B check in this file found): content byte-identical at :968, verified by re-reading the target line.
  "orchestrator/README.md :: dispatcher.js:643-656", // re-pinned from :634-648 -- action 11.1 fix pass (D3): the same sibling fact as doc/state-machine-spec.md's own entry above, missed in the first pass; :634 is a blank line, :635 is "if (childrenSignalled && outcome === 'crashed') {". Re-pinned again for this chantier's own A5b-2 fix pass, F3, same reason and same shift as the doc/state-machine-spec.md entry above.
  "orchestrator/README.md :: doc/state-machine-spec.md:382", // re-pinned from :150, then :159, then :200 (card #212 C1), then :263 (card #212 C2), then :308, then :311 -- card #212's fix pass (F1/F2/F3/F6, 45 net lines) landed above the step table, a true pure shift; content byte-identical at :308 (the CHECK row). Re-pinned again in card #212's C4/C5 build (Lot, 2026-09-14): :311 -> :380, a pure +69-line shift when doc/state-machine-spec.md gained the `continue` verb's own "Resume at CHECK" subsection above the step table; content byte-identical at :380, verified by re-reading the target line. Closing testimony corrected for card #239 A9 (the value had moved :380 -> :382 without this sentence being told, which the Rule B check in this file found): content byte-identical at :382, verified by re-reading the target line.
  "orchestrator/README.md :: intake.js:958-960", // re-pinned from :797-799, then :854-856 by issue #196's own action (+57 lines), then :869-871, then :906-908 by the Priority-as-a-field action (+37 lines); issue #198's resolveLabelArgs extraction (2026-09-12) added a net 30 lines to intake.js above triageBugReport's own header comment (the new shared label-inventory helper used by fileCard and amendCard, netted against fileCard's shrunk header/inlined-label-code and amendCard's grown header). A TRUE pure shift: content byte-identical at :936-938, diffed line by line against the pre-action file. Re-pinned again in card #214 (Lot 9, 2026-09-13): :936-938 -> :938-940, a pure +2-line shift when that action's journalIntakeLlmCall fix (removing `numTurns: raw.numTurns,` and adding a 3-line replacement comment explaining why) landed above this point in the same file; content byte-identical at :938-940, verified by re-reading the target lines. Re-pinned again in card #218 (Lot 12, 2026-09-14): :938-940 -> :953-955, a net +15-line shift when that action's resolveLabelArgs fix (issue #218: replacing the binary `flag === '--label' ? ... : ...` ternaries with the frozen, throwing LABEL_ARGS_BY_FLAG lookup, plus the corrected header-comment sentence) landed above this point in the same file; content byte-identical at :953-955, verified by re-reading the target lines. Re-pinned again for action A2 (card #239, 2026-09-17): :953-955 -> :958-960, the SAME +5-line shift as the doc/state-machine-spec.md entry above; content byte-identical at :958-960, verified by re-reading the target lines.
  ...
  "orchestrator/auto-triage.js :: state-machine.js:3436", // re-pinned from :3402 -- the no-worktree-change fix (readWorktreeHead plus the pre-attempt HEAD read in handleImplement, +34 net lines) landed above this precedent in the same file, a true pure shift; content byte-identical at :3436, verified by re-reading the target line. Before that, re-pinned from :3006, then :3105, then :3171 (Lot 10, card #212's build), then :3276 (card #212 C1), then :3304 (card #212 C2), now :3392 (the comment line itself, as at HEAD :3276, not the appendDaemonEvent call below it) -- card #212's fix pass (F1/F2: the prior-state.json capture, the buildCtx skipWipPreserve field, and the prepareResume-catch's untrusted-path recovery/skip-preserve additions, +58 net lines) landed above this appendDaemonEvent precedent (the `queue-claim-failed` catch) in the same file, a true pure shift; content byte-identical at :3392, verified by re-reading the target line. Previously re-pinned from :2923 by card #174, same pure-shift reason. Closing testimony restated for card #239 A9 (the hops above run newest-first here, so the last clause named the OLDEST value, :3392): content byte-identical at :3436, verified by re-reading the target line.
  "orchestrator/bench-queue-wait.js :: SPO-WebClient/src/e2e/bench/job.ts:325",
  "orchestrator/config.js :: worker.ts:1542",
  "orchestrator/dispatcher.js :: daemon.js:658", // re-pinned from :607, then :637, then :646, then :650 (action A2, card #239) -- see this file's own prior history at each hop. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :650 -> :656, a pure +6-line shift when F3's own daemon.js header edit (correcting the "--scanner split" rationale's now-stale "BLOCKING spawnSync" claim to "STALE SINCE, NOT RE-VERIFIED") landed above this point. Re-pinned once more in the SAME fix pass's second sweep round: :656 -> :658, a pure +2-line shift when that round's own daemon.js dry-run-artifact comment fix (the argv/`query()` options correction) landed above this point too; content byte-identical at :658 (the "CARD #78" `includeReparking` comment), verified by re-reading the target line.
  "orchestrator/dispatcher.js :: daemon.js:677-678", // re-pinned from :626-627, then :656-657, then :665-666, then :669-670 (action A2, card #239) -- see this file's own prior history at each hop. Re-pinned again for this chantier's own A5b-2 fix pass, F3 (card #239, 2026-09-17): :669-670 -> :675-676, the SAME +6-line shift as the daemon.js:650 -> :656 re-pin above. Re-pinned once more in the SAME fix pass's second sweep round: :675-676 -> :677-678, the SAME +2-line shift as the daemon.js:656 -> :658 re-pin above; content byte-identical at :677-678 (`process.once('exit', () => {` / the `killAllChildren('SIGTERM')` line inside it), verified by re-reading the target lines.
  "orchestrator/invariants.js :: doc/state-machine-spec.md:382", // re-pinned from :150, then :159, then :200 (card #212 C1), then :263 (card #212 C2), then :308, then :311, then :380 (card #212 C4/C5, +69 net lines) -- same shift/reason as orchestrator/README.md's own entry above
  "orchestrator/invariants.js :: relative/path/to/file.ts:123",
  ...
  "orchestrator/park-loop.js :: intake.js:958-960", // re-pinned from :797-799, then :854-856 by issue #196's own action (+57 lines), then :869-871, then :906-908 by the Priority-as-a-field action (+37 lines); issue #198's resolveLabelArgs extraction (2026-09-12) added a net 30 lines to intake.js above triageBugReport's own header comment (the new shared label-inventory helper used by fileCard and amendCard, netted against fileCard's shrunk header/inlined-label-code and amendCard's grown header). A TRUE pure shift: content byte-identical at :936-938, diffed line by line against the pre-action file. Re-pinned again in card #214 (Lot 9, 2026-09-13): :936-938 -> :938-940, a pure +2-line shift when that action's journalIntakeLlmCall fix (removing `numTurns: raw.numTurns,` and adding a 3-line replacement comment explaining why) landed above this point in the same file; content byte-identical at :938-940, verified by re-reading the target lines. Re-pinned again in card #218 (Lot 12, 2026-09-14): :938-940 -> :953-955, a net +15-line shift when that action's resolveLabelArgs fix (issue #218: replacing the binary `flag === '--label' ? ... : ...` ternaries with the frozen, throwing LABEL_ARGS_BY_FLAG lookup, plus the corrected header-comment sentence) landed above this point in the same file; content byte-identical at :953-955, verified by re-reading the target lines. Re-pinned again for action A2 (card #239, 2026-09-17): :953-955 -> :958-960, the SAME +5-line shift as the doc/state-machine-spec.md entry above; content byte-identical at :958-960, verified by re-reading the target lines.
  "orchestrator/state-machine.js :: auto-pull.js:58-66",
  "orchestrator/state-machine.js :: auto-pull.js:58-66",
  "orchestrator/state-machine.js :: orchestrator/steps/llm.js:1113", // card #213 action 2 (2026-09-12): handleImplement's own comment on why planDeclaresRdoMembers/diagnoseOrValidateRetry are assigned onto ctx.task before the call cites llm.js's `resolveStepContract(stepName, ctx.task || {})` call site. Cited by the FULL path (not the bare basename "steps/llm.js") -- the citation resolver's basename fallback did not resolve the bare form; measured against the real corpus, only the full path resolves. Re-pinned across card #207, #214 (twice), action A5a, and action A5b (see this file's own prior history at each hop, up to :1009). Re-pinned once more for this chantier's own A5b-2 fix pass, F2 (card #239, 2026-09-17): :1009 -> :1011, a pure +2-line shift when F2's correction to the `suppliedSessionId`-block comment landed above this call site in llm.js. Re-pinned once more for this chantier's own action A6 (card #239, 2026-09-17): :1011 -> :1035, a pure +24-line shift when A6 added the `../live-progress` require above invokeClaudeReal, its own onMessage/finally-clear block inside invokeClaudeReal, and the `taskDir: ctx.taskDir,` field (with its own comment) to runLlm's legacy override branch -- all landing above this call site in the same file. Re-pinned once more for this chantier's own action A7 (card #239, 2026-09-17): :1035 -> :1065, a pure +30-line shift when A7's ruling on `maybeRecoverTokens` (ruled load-bearing, not dead code -- see that function's own comment) added a measured-evidence paragraph above this call site in the same file. Re-pinned once more for A7's OWN Opus verifier fix pass, same day: :1065 -> :1105, a pure +40-line shift when the fix pass corrected the ruling's corpus figure (F1: 117/12.8%/"dominant shape" was the retroactive backfill's own writes, not live recoveries -- corrected to 25/810/3.09%, all `ok:false`) and downgraded the architectural claim from measured to a named structural inference (F2) -- both landing above this call site in the same file. Re-pinned once more for the CI fix pass that closed out the 8 dry-run-demo/regression failures on CI run 35620555644 (card #239, 2026-09-22): :1105 -> :1113, a pure +8-line shift when that fix added the `DRY_RUN_CLAUDE_UNRESOLVED_PLACEHOLDER` constant (steps/llm.js, near ZERO_TOKENS) above this call site so runLlm's dry-run branch stops requiring a REAL `claude` on PATH (see that constant's own comment, and runLlm's dry-run branch, for why the old unconditional `buildQueryOptions` throw made every dry run depend on the host, which CI's runner does not have); content byte-identical at :1113 (still `const contract = resolveStepContract(stepName, ctx.task || {});`), verified by re-reading the target line.
  ...
  "orchestrator/steps/sdk-call.js :: llm.js:1060", // F2 fix pass: the module header cites llm.js:1002 (`jsonSchema: override.jsonSchema,` inside runLlm's legacy `ctx.task.llm.<step>` override branch) as the live evidence that opts.jsonSchema's string shape is real, not hypothetical. Confirmed by reading orchestrator/steps/llm.js:1002 directly. Resolved by basename (no directory prefix in the citation text) -- unambiguous, since llm.js exists in this repo only under orchestrator/steps/. Re-pinned across action A5a and A5b (see this file's own prior history at each hop, up to :962). Re-pinned once more for this chantier's own A5b-2 fix pass, F2 (card #239, 2026-09-17): :962 -> :964, the SAME +2-line shift as the state-machine.js :: llm.js entry above (F2's `suppliedSessionId`-block comment correction). Re-pinned once more for this chantier's own action A6 (card #239, 2026-09-17): :964 -> :982, a pure +18-line shift from two edits landing ABOVE this line in llm.js -- the `../live-progress` require (+1) and invokeClaudeReal's own onMessage/finally-clear block (+17, replacing a 6-line try/finally with a 22-line one) -- while A6's OTHER edit to this same file, the `taskDir: ctx.taskDir,` field added to runLlm's override branch, lands AFTER this line and so contributes nothing to its shift. Re-pinned once more for this chantier's own action A7 (card #239, 2026-09-17): :982 -> :1012, the SAME +30-line shift as the state-machine.js :: llm.js entry above (A7's `maybeRecoverTokens` ruling paragraph). Re-pinned once more for A7's OWN Opus verifier fix pass, same day, the SAME +40-line shift as the state-machine.js :: llm.js entry above (F1's corpus-figure correction plus F2's measured-vs-inferred downgrade): :1012 -> :1052. Re-pinned once more for the CI fix pass that closed out the 8 dry-run-demo/regression failures on CI run 35620555644 (card #239, 2026-09-22), the SAME +8-line shift as the state-machine.js :: llm.js entry above (the new `DRY_RUN_CLAUDE_UNRESOLVED_PLACEHOLDER` constant): :1052 -> :1060; content byte-identical at :1060 (`jsonSchema: override.jsonSchema,`), verified by re-reading the target line.
  "orchestrator/steps/sdk-call.js :: llm.js:1060", // same target, cited a second time at the jsonSchema-parsing block's own comment further down this same file -- occurrences count separately here, same convention as the orchestrator/README.md:247 pair below. Re-pinned the same way and for the same reason as the row above, at every hop, up to and including :1052 -> :1060 for the CI fix pass.
...
//      within one un-clipped span -- `account-lease.js:167`, `dispatcher.js:643-656`, and
...
const CITATION_ANCHOR_ALLOWLIST = {
  // "...the same write-tmp-then-`linkSync` `tryCreate` daemon.lock uses too (`account-lease.js:167`
  ...
  // account-lease.js:167's own content (`tryAcquireLease`'s closing brace, genuinely unnamed in
  // this prose). Confirmed correct: line 167 is exactly where `tryAcquireLease`
  // (orchestrator/account-lease.js) calls `lock.acquireShortLock` and closes. Re-pinned from :156
  // for action A2 (card #239, 2026-09-17): ~11 net lines added to the "why 2x"/"roughly 7.6x"
  // paragraphs above tryAcquireLease in the same file, a true pure shift; content byte-identical
  // at :167, verified by re-reading the target line.
  'orchestrator/README.md :: account-lease.js:167':
    ...
    '167 is where tryAcquireLease calls lock.acquireShortLock and closes.',
...
const EXPECTED_DISCRIMINATING_CITATIONS = [
  ...
  'orchestrator/state-machine.js :: orchestrator/steps/llm.js:1113',
  'orchestrator/state-machine.js :: run.ts:63',
  'orchestrator/state-machine.js :: step-contracts.js:1102',
  'orchestrator/steps/scripted.js :: run.ts:63',
  'orchestrator/steps/sdk-call.js :: daemon.js:111',
  'orchestrator/steps/sdk-call.js :: llm.js:1060',
  'orchestrator/steps/sdk-call.js :: llm.js:1060',
  'prompts/README.md :: plan.md:103',
];
...
test('MUTATION PROOF, corpus-wide: every single-line citation the anchor check accepts stops anchoring when its line is shifted by one', () => {
  ...
  // classifies UNANCHORABLE instead (`orchestrator/park-loop.js :: intake.js:953-955` -- this
...
const EXPECTED_LIVE_PIN_KEYS = [
  ...
  "doc/state-machine-spec.md :: intake.js:958-960 @ HEAD",
  ...
  "orchestrator/README.md :: intake.js:958-960 @ HEAD",
...
test('Rule B is live against the real registry: re-introducing a stale closing clause turns it red', () => {
  ...
  const target = entries.find((e) => e.key === 'orchestrator/README.md :: config.js:968');
  ...
  const mutated = entries.map((e) =>
    e === target ? { key: e.key, prose: e.prose.replace(/content byte-identical at :968/g, 'content byte-identical at :966') } : e
  );
