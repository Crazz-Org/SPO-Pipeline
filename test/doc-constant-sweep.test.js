...
test('Rule B is live against the real registry: re-introducing a stale closing clause turns it red', () => {
  ...
  const target = entries.find((e) => e.key === 'orchestrator/README.md :: config.js:1174');
  ...
  const mutated = entries.map((e) =>
    e === target ? { key: e.key, prose: e.prose.replace(/content byte-identical at :1174/g, 'content byte-identical at :968') } : e
  );
