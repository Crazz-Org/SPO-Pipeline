...
test('Rule B is live against the real registry: re-introducing a stale closing clause turns it red', () => {
  ...
  const target = entries.find((e) => e.key === 'orchestrator/README.md :: config.js:1127');
  ...
  const mutated = entries.map((e) =>
    e === target ? { key: e.key, prose: e.prose.replace(/content byte-identical at :1127/g, 'content byte-identical at :968') } : e
  );
