...
test('card #78 (fix): a graceful drain WAITS for an in-flight repark to finish its own work, instead of handing it to the (much shorter) reap grace', { timeout: 20000 }, async () => {
  ...
  // and 'dispatcher-drain-end' being journalled (dispatcher.js:1309 -- card #219's fix pass
  // corrected a stale second line number here, which named nothing real; killAllChildren itself,
  // named accurately two paragraphs below, is the citation that matters), and
  ...
  //
  // 'dispatcher-stopped' (dispatcher.js:1255) is the anchor that DOES discriminate: it is journalled
  // the instant `awaitInFlight` returns, strictly BEFORE `killAllChildren`/the reap (dispatcher.js
  // :1286-1287) ever run -- so in the buggy mutant above, 'dispatcher-stopped' is written before the
...
// restarts (a deploy -- `git pull` in the deploy checkout, never a GitHub merge alone -- runs the
// post-merge hook, which drain-restarts the unit; KillMode=mixed SIGTERMs the dispatcher directly),
// and the pool then recovers, the `returned` edge is never written -- leaving a bare idle edge as
// daemon.jsonl's newest dispatcher event forever. console/dispatcher-status.js's
// computeDispatcherStatus (renamed by card #164; was computeDispatcherIdleStatus; moved out of
// bin/spo by card #186 so `spo status` and the dashboard deck share one derivation) answers "is
// the dispatcher idle right now" by
