...
// F5's own --deadline-ms help-text fix added 4 lines above it in daemon.js's printUsage), 1273
...
// lock.js replays use.
test('historical drift replay: daemon.js -- old citation :626-627 (its real position until a483724 added ~39 lines above the exit hook, then action A2 added 4 more; unchanged text) resolves ok with a correction to the real current lines', () => {
  const pin = {
    file: 'test replay',
  ...
  assert.equal(result.ok, true, `expected ok: true: ${result.why}`);
  assert.ok(result.correction, 'expected a correction to be reported');
  assert.equal(result.correction.start, 669);
  assert.equal(result.correction.stop, 670);
  assert.equal(result.correction.citation, 'orchestrator/daemon.js:669-670');
});
