...
| DIAGNOSE | `diagnose.md` | Opus 5.5 since 2026-09-23 (Opus 5 before; moved off Fable 5 on 2026-09-04 — half the token price, and four of five steps on Fable meant one Fable-only quota limit cooled the whole account for every model; not a quality change, DIAGNOSE was 8/8 post-C1) | high | `Read, Grep, Bash(ro)` | reads `{{diff_path}}`, `{{gate_log_path}}`, `{{ledger_path}}` |
