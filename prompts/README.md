...
changes for the task's shape; it changes only on a Fable model limit, SPO-Pipeline#166).

**A usage limit changes the model on exactly two steps**, VALIDATE's citation-verifier and
change-validator (Fable 5 → Opus 5.5, on a Fable **model** limit only). Every other step — PLAN,
