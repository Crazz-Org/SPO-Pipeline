...
|---|---|---|---|---|---|
| PLAN | `plan.md` | **Opus first, Fable 5 as fallback** since 2026-09-13 — Opus 5.5 (`claude-opus-5-5`) since 2026-09-23, Opus 5 before (maintainer decision, for cost — `EXP-PLAN-OPUS` in `doc/model-experiments.md`, adopted 2026-09-24 on its Opus 5 arm; the Opus 5.5 base is on trial as `EXP-PLAN-OPUS-5-5`). The one trigger is `planInvalidRetry`, which `handlePlan` sets in two cases: (a) after a real-mode Opus reply that would park `plan-invalid`, for one more call on Fable in the same run; (b) from the first call, when the card's most recent park was `plan-invalid`. Each switch journals `PLAN/plan-model-fallback`. Transport failures and deadline kills never fall back, because they say nothing about the plan. Before 2026-09-13: Fable 5 with no escalation (an earlier "Opus 5 fallback" hung on `task.escalate`, which nothing set, and was removed 2026-09-04) | per task `Size` through `PLAN_EFFORT_BY_SIZE`: S/M/L → medium/high/high (one rung above the old low/medium/high, adopted with it) | `Read, Grep, Glob, Bash(ro)` | reads `{{worktree}}`; holds no write tool at all -- returns `plan_markdown`/`invariants_markdown`, the driver writes both under `{{scratch_dir}}`; also returns `files_to_change` (action 3.2 — **absolute** paths under `{{worktree}}`, `plan.md:103`, distinct from paths it merely reads or cites) |
| IMPLEMENT | `implement.md` | **Opus 5.5** (`claude-opus-5-5`) on every path since 2026-09-23 (experiment `EXP-IMPLEMENT-OPUS-5-5`, `doc/model-experiments.md`; Sonnet 5 escalating to Opus 5 before). Its four triggers now escalate **effort** to `medium`, not model (card #213 action 2 + its 2026-09-12 amendment, resolved by `step-contracts.js`'s `escalationSignalFires`, `escalatesEffortOn: ['planDeclaresRdoMembers', 'lSize', 'diagnoseOrValidateRetry']`): (1) `task.rdoDiffTouched === true`, the real diff once PUSH_PR has run; (2) `task.planDeclaresRdoMembers`, PLAN's own `files_to_change` declaration naming `rdo-members.ts` (resolved fresh at each call by `state-machine.js`'s `resolvePlanDeclaresRdoMembers` — an EMPTY declared list still counts and resolves `false`, with no fallback to (3)); (3) `task.touchesRdoMembers === true`, set once at intake from the issue's Area field or a literal `rdo-members.ts` mention in its body — read ONLY when (2) is undefined (PLAN never declared a list at all), which is where the old "narrower than the full wire rule" / false→true-promotion story below still applies; or (4) `task.diagnoseOrValidateRetry === true` — a retry after a DIAGNOSE or a VALIDATE reject, independent of the RDO signals, escalating on observed difficulty. `touchesRdoMembers` is still promoted false→true, never lowered, by PUSH_PR (which runs AFTER IMPLEMENT's first pass) when the real diff touches `src/shared/rdo-members.ts` — armed for the IMPLEMENT retries that follow within that SAME worker run (the promotion lives in `ctx.task`, never written back to `task.json`, so a crash/repark or a `retry` re-enqueue rebuilds `ctx.task` from `task.json`'s original intake guess and loses it) — but by the time any such retry happens, sources 1/2 above usually already have an answer and take priority; source 3 is the fallback for a card that has reached neither. Also escalates on an `L`-sized task. | per `Size` through `IMPLEMENT_EFFORT_BY_SIZE`: S/M/L → low/medium/medium; `medium` whenever a trigger fires | full edit tools | reads and writes `{{worktree}}` only |
| DIAGNOSE | `diagnose.md` | Opus 5.5 since 2026-09-23 (Opus 5 before; moved off Fable 5 on 2026-09-04 — half the token price, and four of five steps on Fable meant one Fable-only quota limit cooled the whole account for every model; not a quality change, DIAGNOSE was 8/8 post-C1. That second reason no longer holds as stated: card #167 made the cooldown per `(account, model)`, so a Fable limit cools Fable alone. The move itself stands on the price argument, and on concentration — a pool-wide Fable exhaustion still stalls every Fable step at once — SPO-Pipeline#166 answered that for the two VALIDATE judges only, which fall back to Opus 5.5 once no account has Fable quota left (a 529 overload doesn't count), onto an account with Opus 5.5 quota (#277); review-card and PLAN's Fable fallback still wait) | high | `Read, Grep, Bash(ro)` | reads `{{diff_path}}`, `{{gate_log_path}}`, `{{ledger_path}}` |
| VALIDATE — citation-verifier | `verify-citations.md` | Fable 5; **Opus 5.5 once no account has Fable quota left — a 529 overload doesn't count** (SPO-Pipeline#166/#277 — see the change-validator row) | high | `Read, Grep` (product + `~/SPO-Original`, read-only) | reads the diff and the server-side Pascal declarations under `{{spo_original_path}}` (today: `Kernel/`; `Rdo/Server/` is the RDO transport layer — dispatch machinery, not the game-object declarations a catalogue entry cites); runs only when the REAL diff touched the RDO catalogue — PUSH_PR's diff-derived `task.rdoDiffTouched`, resolved by `state-machine.js`'s `resolveRdoDiffTouched`, not intake's `touchesRdoMembers` guess — and always before `validate-change.md` |
| VALIDATE — change-validator | `validate-change.md` | Fable 5 — never the executor's model (Opus 5.5 since 2026-09-23, Sonnet 5 before — the executor may not judge itself), and never Opus either: the wire rule escalates **effort**, not model (Fable is the more capable tier, so `fable → opus` made the judge weaker exactly where the stakes are highest — observed on card #462). **The quota exception** (SPO-Pipeline#166, maintainer decision 2026-09-24): when no enabled account has Fable QUOTA left (a model limit, a session or weekly window, a record with no kind — a 529 overload doesn't count: that account's Fable is back within minutes, so the judge waits), onto an account that still has Opus 5.5 quota — a limit on one account first rotates on Fable to an account that still has it, and a pool where every account is account-wide limited has no Opus 5.5 either, so it waits (SPO-Pipeline#277, 2026-09-25) — the call retries on Opus 5.5 (`quotaFallbackModel`, same effort) instead of waiting, so the judge sometimes grades its own model's work; journalled `model-fallback`, the verdict marked `quotaFallback: true`, measured as `EXP-JUDGE-QUOTA-FALLBACK` in `doc/model-experiments.md` | high, **xhigh** when the REAL diff touched the RDO catalogue (`task.rdoDiffTouched`, not intake's `touchesRdoMembers` guess -- card #213) | `Read, Grep, Glob, Bash(ro)` | reads `{{diff_path}}`, `{{invariants_path}}`, `{{gate_report_path}}`, `{{pr_body_path}}`, and every file in `{{scoped_claude_md_paths}}` |
...
2026-09-23), and `validate-change.md` states the **effort** escalation (xhigh — its model never
changes for the task's shape; it changes only when no account has Fable quota left,
SPO-Pipeline#166/#277).

**A usage limit changes the model on exactly two steps**, VALIDATE's citation-verifier and
change-validator (Fable 5 → Opus 5.5, only once no enabled account has Fable quota left (a 529
overload doesn't count), and onto an account that has Opus 5.5 quota — a limit on one account rotates on Fable
first, SPO-Pipeline#277). Every other step — PLAN,
IMPLEMENT, DIAGNOSE and the three intake prompts — never changes model on a limit: it rotates to
another account on the same model, then waits. PLAN's Fable fallback is a plan-quality fallback,
not a quota one. The rule and its two triggers: `doc/state-machine-spec.md` § Step contracts,
*Which steps may change model on a limit*.
...
- **`{{scoped_claude_md_paths}}`** (VALIDATE) — the scoped `CLAUDE.md` files above the files the
  diff changes (`src/client/CLAUDE.md`, ...), absolute and comma-separated, or a fixed "none"
  sentence; derived from `diff.patch` by `task-values.js`'s `scopedClaudeMdPaths` (card 52).
  VALIDATE runs from this repo's root, so it loads none of them on its own. `{{pr_body_path}}`
  (VALIDATE) is the fixed `journal/<id>/pr-body.md` PUSH_PR wrote.
