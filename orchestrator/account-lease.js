...
//                       resolveStepContract(stepName, ctx.task).model (the same lookup
//                       steps/llm.js re-runs for the actual invocation, so the lease and the
//                       spend can never disagree), intake.js from each intake function's own
//                       hardcoded literal. Omitted (any caller not yet threaded through) keeps
