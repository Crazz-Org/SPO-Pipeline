...
- `api_error_status === 429` (the definitive rate-limit status, **observed**: the only recorded
  real limit in this repo, `intake.js:938-940`'s 12.8-hour Fable incident — "You've reached your
...
uses, and, since the measured 39%-torn-read defect (`lock.js:278-309`: 119 of 800 cooldown
entries lost), the same write-tmp-then-`linkSync` `tryCreate` daemon.lock uses too
(`account-lease.js:156` → `lock.js:276` `acquireShortLock` → `:310` `tryCreate`) — a bare `'wx'`
