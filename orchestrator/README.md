...
- `api_error_status === 429` (the definitive rate-limit status, **observed**: the only recorded
  real limit in this repo, `intake.js:889-891`'s 12.8-hour Fable incident — "You've reached your
...
Every draft carries a `priority`, one of **`CRITICAL` / `HIGH` / `MEDIUM` / `LOW`**. It is a
...
maintainer's 2026-09-11 criticity review did exactly that, by hand, for 18 issues. The four words
of the vocabulary are the ones that corpus already used (#161 `CRITICAL`, #162 `HIGH`, #165
`MEDIUM`, #198 `LOW`), deliberately not GitHub's P0/P1/P2 template, so the backfill of the
existing board was lossless and each card's own prose still agrees with its field.
...
| | |
|---|---|
| `CRITICAL` | Losing work, money or availability **now**, on a trigger needing nothing rare. |
| `HIGH` | Routine trigger, costly outcome. Next to schedule once CRITICAL is clear. |
...
(`priority: HIGH`, upper-cased on apply), and `fileCard` returns the **applied** value — so what
lands on the board is the judge's verdict, not the drafter's first guess.

**`DECISION` is not a priority.** "A human must arbitrate before any code is written" is an
orthogonal axis: a DECISION card can be CRITICAL (#166) or LOW (#79). It stays a title prefix, and
`VALID_PRIORITIES` rejects it as a value on purpose.
...
- The **value** is not one the field offers — a typo, or a vocabulary that drifted — fails loudly
  *before* any board mutation. Silently filing a CRITICAL card as untriaged is the failure this
...
`PVTSSF_lADOEyAVD84BiHMrzhiG-_Q`; options CRITICAL `c38699dc`, HIGH `fc35ba09`, MEDIUM
`4fe07203`, LOW `283ea466` — re-read them with `gh project field-list 2 --owner Crazz-Org --format
json` rather than trusting these if a call 404s. The mutation is the same
