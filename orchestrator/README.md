...
> **Card #214 added a `modelUsage` field to this same result** (and to the `llm-call` event
> steps/llm.js journals from it) -- not shown in the example above because a single-model call
> like the one shown carries it as `{ '<model-name>': { freshInputTokens: 120,
> cacheCreationTokens: 0, cacheReadTokens: 0, outputTokens: 8, billableTokens: 128 } }`, i.e. the
> same four fields (plus `billableTokens`) broken out per model instead of summed. See
> extractTokens's own header comment in `orchestrator/steps/llm.js` for the exact shape and the
...
not one of the eight reasons that indict the plan itself (`plan-invalid`,
...
Either way, the `cat:<category>`/`size:<size>` labels fileCard requests are conditional on the
target repo's own label inventory (`gh label list`, read just before `gh issue create`): a label
...
PER MODEL rather than summed across every model the call touched), so there is no
second ledger to keep in sync. `modelUsage` exists because the event's own `model` field names
only the step's CONTRACT model: PLAN is measured to spawn Opus subagents (6 of 42 calls,
measured 2026-09-10..12 while PLAN was Fable-only — PR #222 changes PLAN to Opus-first with a
Fable fallback) while resolving to Fable itself (`step-contracts.js`'s comment
on PLAN's `allowedTools`, which also carries the corrected, fuller-corpus measurement —
IMPLEMENT can delegate too, not only PLAN), and without the per-model breakdown those subagents' tokens were indistinguishable
from the resolved model's own once summed into the flat totals. `spo tokens` prints it per task (state, calls, the four token
...
behind it is taken until it is. Such a hold is journalled once per episode (`dispatcher-hold`,
naming the card, its model and the `idleAccounts` it leaves unused) and closed by
`dispatcher-hold-cleared` (SPO-Pipeline#269) — edge-triggered, like the idle edge, so a hold is
told apart from "every slot is full" in `daemon.jsonl`. Until #166
...
| `dispatcher-drain-start` | daemon | a SIGTERM/SIGINT asked the dispatcher to drain instead of killing: records the `signal`, the `timeoutMs` bound (`config.drainTimeoutMs`), the `inFlight` card ids it is about to wait for, and (card #188) `pid` — the same `process.pid` `dispatcher-start` carries, added so a reader can tell this drain's process apart from any other without depending on the matching `dispatcher-start` still being inside its own bounded read window — and `reason`, read off the in-memory `stopReason` at the moment this is written (`(stopReason && stopReason.reason) || null`): normally `'drain-requested'` (`requestDrain` only ever sets `stopReason` when nothing set it first), but a `stop()` call landing before `requestDrain` did leaves ITS reason here instead, so a reader never has to assume every drain was requested. Also (card #188 follow-up, 2026-09-11) `killGraceMs` — `resolveDrainKillGraceMs(config)`, the SAME resolver the reap below this write is handed — so a reader can bound an unconcluded drain's own age using the grace THIS process's config actually applies, rather than assuming the reader's own config matches the writer's. Also (card #208) `hostUptimeAtMs` — `os.uptime() * 1000`, seconds since boot converted to ms — because this event is written by the daemon and read by a DIFFERENT process (`spo status`, the dashboard), and `os.uptime()` is both monotonic (unlike `ts`) and comparable across processes on the same boot on EVERY platform, which a bare `hrtime.bigint()`/`monotonicNowMs()` reading (`orchestrator/monotonic-clock.js`) is not guaranteed to be — Node's own docs promise only "an arbitrary time in the past". Also (card #219, 2026-09-14) `monotonicAtMs` — `monotonicNowMsFn()`, the SAME elapsed-clock reading `awaitInFlight` itself waits on, captured here as an absolute value at write time — carried ALONGSIDE `hostUptimeAtMs`, never instead of it: measured on this host (Linux/WSL2, Node v22) two separate processes' `hrtime.bigint()` reads 56ms apart differed by 56.4ms and both tracked `/proc/uptime`, i.e. on Linux libuv's `uv_hrtime()` IS the system-wide `CLOCK_MONOTONIC` and comparable across processes on the same boot — a measured LINUX IMPLEMENTATION DETAIL, not a cross-platform Node guarantee, so a reader trusts it only behind a plausibility check (below) and falls back to `hostUptimeAtMs` alone everywhere else. `ts` is kept alongside both, unchanged, as the human-readable record. Claiming has already stopped by the time this is written — the scanner, the only producer of new queue entries, is signalled inside `requestDrain` (`dispatcher.js`). Best-effort (wrapped in its own try/catch): a throw here must not skip the wait it precedes. Card #188: of the event kinds `computeDispatcherStatus`'s own precedence walk reads (`dispatcher-healthy-accounts-returned`/`dispatcher-start`/`dispatcher-drain-start`/`dispatcher-stopped`/`dispatcher-idle-no-healthy-accounts`, and since SPO-Pipeline#269 `dispatcher-hold-cleared`/`dispatcher-hold` — worker/scanner exits and reparks are NOT among them), this is the only one written between a drain being requested and it concluding (`dispatcher-stopped`/`dispatcher-drain-end`, both written only after the wait resolves) — a process that dies inside that wait leaves this as the newest dispatcher edge that walk can see, until the next `dispatcher-start`. `console/dispatcher-status.js`'s `computeDispatcherStatus` reads an unconcluded one as its own state: (card #188 follow-up, card #208, card #219) FIRST, a bound, on whichever age reading the event carries, tried PREFERRED-MONOTONIC, then PREFERRED, then LEGACY. PREFERRED-MONOTONIC, when `monotonicAtMs` is present and a plausibility check (both readings finite, no reboot, `monotonicNowMs >= monotonicAtMs`, and monotonic elapsed no more than boottime elapsed plus a small tolerance — see that module's own header for the full reasoning) holds: `monotonicNowMs` (injected) minus `monotonicAtMs`, compared against `timeoutMs` plus grace — closes the suspend residual named below WHERE that check holds (measured true on Linux; assumed false, falls back, elsewhere). PREFERRED, when `hostUptimeAtMs` is present and the monotonic path above did not apply: `hostUptimeNowMs` (injected by the caller, `os.uptime() * 1000`) minus the event's own `hostUptimeAtMs`, compared against `timeoutMs` plus a grace (the event's own `killGraceMs` if present, else the caller's injected `killGraceMs`) — `hostUptimeNowMs` reading LESS than `hostUptimeAtMs` means a reboot happened since the write, read as `'stopped'`/`diedDraining: true`/`rebooted: true` unconditionally (isAlive is never consulted — a pre-reboot pid cannot answer whether THIS process is alive). LEGACY, only for a record with no `hostUptimeAtMs` at all (written before card #208): `now` (injected by the caller) minus the event's own `ts`, compared the same way. Either path, past its own bound the reading is `'stopped'`/`diedDraining: true` unconditionally, because a drain that ran as designed writes `dispatcher-stopped` the instant the wait itself resolves, before any kill or reap; each path applies only when every input it needs is a real number, so a caller (or a legacy record) missing any of them skips that path — the PREFERRED path never falls back to the LEGACY one; only an event with no `hostUptimeAtMs` at all uses it. Short of either bound (or with neither available), liveness decides exactly as before: `'draining'` while the process is alive or its liveness cannot be checked, by `isAlive(pid)`; `'stopped'` with `diedDraining: true` once the liveness probe reports it gone — rather than falling through to `null` the way it used to before card #188. Residual gaps, updated by card #219 (2026-09-14): a drain still WITHIN its own bound whose pid has since been reused (by an unrelated process started AFTER the drain began -- the reuse check runs only inside the bound, after the bound itself has already found no reason to say stopped) now reads `'stopped'`/`diedDraining: true`/`pidReused: true` instead of `'draining'` forever, when `isAlive(pid)` says alive AND `orchestrator/lock.js`'s `processStartUptimeMs(pid)` (Linux `/proc/<pid>/stat`, card #219) can prove the pid started measurably (`PID_REUSE_SLACK_MS`) after `hostUptimeAtMs` — null (non-Linux, pid gone, unparseable) leaves the previous 'draining' verdict unchanged, so this remains open on non-Linux platforms and whenever the probe cannot read `/proc`. For a LEGACY (no-`hostUptimeAtMs`) record, the bound is wall-clock while the wait is monotonic, so a forward clock step during such a drain can read a live, still-waiting process as `'stopped'` — a record carrying `hostUptimeAtMs` closes that forward-step half of the gap (`os.uptime()` never takes the NTP-style step corrections `Date.now()` has been measured taking on this box). A HOST SUSPEND is now closed by the PREFERRED-MONOTONIC path above WHERE its plausibility check finds hrtime trustworthy — measured true on Linux (this host, WSL2, see above), assumed false (path skipped, falls back to PREFERRED-uptime, suspend residual remains) elsewhere: `/proc/uptime` (`CLOCK_BOOTTIME`) counts suspended time, `CLOCK_MONOTONIC` (hrtime) does not, and outside the measured-Linux case nothing here re-derives that guarantee at runtime beyond the plausibility check's own internal-consistency test. A live suspend was still not reproduced on this host in card #219 either, for the same reason card #208 could not: `CLOCK_BOOTTIME - CLOCK_MONOTONIC` held at -1.7 microseconds after 63 hours of this box's own uptime — consistent with either "never suspended" or "WSL2 collapses the two clocks", so this is closed BY CONSTRUCTION where the clock allows, not demonstrated live — see `doc/accepted-gaps.md` for the platforms and shapes still open. Read by `spo status`'s `dispatcher:` line (injecting `orchestrator/lock.js`'s `pidExists`, not `processAlive` — a foreign-but-alive pid must not read EPERM as dead — plus `now`/`hostUptimeNowMs`/`killGraceMs` off `orchestrator/config.js`'s `drainKillGraceMs` and a fresh `os.uptime() * 1000`, and, since card #219, `orchestrator/lock.js`'s `processStartUptimeMs` and a fresh `orchestrator/monotonic-clock.js`'s `monotonicNowMs()`), the dashboard's Workers tile (`console/collect.js`'s `applyWorkerStats`, same injections), and the Bug Reports drain-history line (`console/render.js`'s `renderReportsInner`, which is handed that same already-computed verdict rather than re-deriving it). |
| `dispatcher-hold` | daemon | SPO-Pipeline#269: the queue head is servable but held — the live workers already number the accounts healthy for its first call's model, so nothing behind it is admitted. Edge-triggered, one per episode (one card on one model): `id`, `step`, `model` (held on), `viaFallback`, `healthy`, `live`, `configuredWorkers`, `queued`, and `idleAccounts` — enabled accounts healthy for some model and holding no live lease at that moment, the throughput the hold costs (`dispatcher.js`). |
| `dispatcher-hold-cleared` | daemon | the hold episode ended: `id`, `model`, `heldMs` (the dispatcher's own monotonic elapsed time) and `taken` (whether the held card itself was admitted on that pass). Written before any spawn or idle edge of the same pass (`dispatcher.js`). |
...
| `dispatcher-stopped` | daemon | action 3.3, hoisted by card #162: no longer written at `run()`'s own return — it now lands right after the drain merges its own fields into `stopReason` and BEFORE `killAllChildren`/`reapSignalledChildren`, so a systemd `TimeoutStopSec` SIGKILL landing during the reap (the first point in `run()` that actually yields) cannot destroy the record of the stop it exists to explain. Still the single point every stop path converges on before anything else happens, so it covers **every** stop path rather than an enumerated set — the four a maintainer meets in production are a drain, `stop-requested`, and the two circuit breakers (`worker-crash-circuit-breaker`, `scanner-crash-circuit-breaker`), but `dispatcher.stop({reason})` is a public method and `recette.js` alone passes four more (`wall-clock-cap-exceeded`, `llm-step-cap-exceeded`, `recette-scenario-complete`, `recette-scenario-teardown`), so read `reason` rather than assuming this list is closed. Carries `stopReason` spread flat (so `reason`, plus whichever fields that particular stop path added — `drained`/`waitedMs`/`survivors`/`signal` for a drain, `consecutiveCrashes`/`crashLimit`/`lastId` for the worker breaker, `consecutiveScannerCrashes`/`totalScannerCrashes`/`scannerCrashLimit` for the scanner breaker) land as this event's own top-level fields. Before this action the single most important event in this subsystem existed only on stderr (`daemon.js`'s own `dispatcher stopped itself -- <JSON>` line, printed by the dispatcher process itself, not journalled) and as the process's own exit code; `daemon.jsonl` could not show it at all (`dispatcher.js`). Now precedes `dispatcher-drain-end` in the journal on a drain path, where it used to follow it. |
...
| `empty-implement` | task | IMPLEMENT's payload declared `files_changed` but the list parsed empty — routes to DIAGNOSE (`state-machine.js`). A `{modified, new, ...}` object of path lists is flattened first (card 51); an empty list WITH a non-empty `stop_reason` parks `implement-stopped` instead (no `empty-implement`) on a first, clean attempt only — no diagnosis ever, clean tree, HEAD on the base sha; otherwise this event is journalled as before, carrying `stopReason`. |
| `queue-claim-failed` | daemon | card #137: `takeNextTask` picked an eligible candidate but `fs.renameSync` (or the `fs.mkdirSync` before it) threw claiming it — a solitary environment failure (`EXDEV` when `queueDir` and `journalRoot` are on different filesystems, `EPERM`/`ENOSPC` on the target), not the ENOENT race two daemons sharing a lock used to produce (`lock.js`'s singleton mutex already eliminated that by removing the second daemon, not by handling the error). The candidate is left in `queue/` untouched and the loop tries the next eligible entry instead of aborting the whole call — so one stuck entry cannot starve the entries behind it, and `drainQueueOnce`/`fillSlots` never see a throw, only a `null` return once every candidate is exhausted (`state-machine.js`). |
...
| `worktree-moved-by-commit` | task | IMPLEMENT left a clean tree but `HEAD` moved during the attempt (`headBefore` → `headAfter`): it committed its own fix, so card #385's cross-check counts the worktree as moved and routes to CHECK. Before this, the porcelain-only check sent such an IMPLEMENT to DIAGNOSE until the card parked (issue-598 `diagnose-budget-exhausted`, issue-593 `diagnose-no-new-cause`) (`state-machine.js`). |
