...
const STEP_CONTRACTS = {
  ...
  IMPLEMENT: {
    ...
    outputContract: {
      ...
      // SPO-Pipeline card 48: IMPLEMENT may propose the commit subject -- a Conventional Commit
      // (`type(scope): summary`), which SPO-WebClient's changelog needs. Optional and untyped on
      // purpose: PUSH_PR (steps/scripted.js's commitSubject) validates it and falls back to a
      // subject derived from the card, so a missing or malformed value can never park a card.
      optional: ['commit_subject'],
    },
  },
