...
| `Session` project field | Ownership marker written by `board-take.sh` (`npm run board:take`, spawned live from `orchestrator/steps/scripted.js:1389` at WORKTREE) on every claim, and read back by its own `--release` path | **Keep** — this field is live and load-bearing for the single orchestrator today, not a legacy holdover. Not touched here. |
...
4. **Reorder the `Status` field options** to the target sequence — open the `Status` field's
   ...
   MUST stay an option: `config.js:1097`'s `reportIntakeColumn` defaults to it, and
