...
| 5.2 | **Cost and duration on the card**: enriched final Done comment (total cost, duration, attempt count — cost is already summed at FINISH); park comments carry cumulative cost + attempt history. | `orchestrator/steps/scripted.js:789`, `orchestrator/park-loop.js` (buildParkComment) |
...
| 7.4 | **Killed-call accounting**: a deadline-killed/E2BIG call journals `costUnknown: true` instead of `costUsd: 0` (the efficiency metric vs. the $12 baseline is currently biased low). | `orchestrator/steps/llm.js:237`, `orchestrator/cost.js` |
