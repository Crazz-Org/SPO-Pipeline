...
| 1.4 | **`realFinish` runs the reinstall rule**, and fast-forwards the main checkout as `doc/state-machine-spec.md:387` already promises. Both, in one action — the reinstall without the fast-forward installs the wrong binary and looks like it worked. | `orchestrator/steps/scripted.js` (`realFinish`), `doc/state-machine-spec.md`, `test/` |
