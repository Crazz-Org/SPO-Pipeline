...
| 3.5 | **The rate limiter that cannot fire.** `config.ts:93,98` set `minIntervalMinutes: 0` and `maxRunsPerDay: 1000`, so `BLOCKED`'s rate-limit path is unreachable. Set real values or remove the path and its documentation. | `src/e2e/config.ts`, `src/e2e/run.ts` |
