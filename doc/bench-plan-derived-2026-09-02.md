...
| 1.4 | **`realFinish` runs the reinstall rule**, and fast-forwards the main checkout as `doc/state-machine-spec.md:389` already promises. Both, in one action — the reinstall without the fast-forward installs the wrong binary and looks like it worked. | `orchestrator/steps/scripted.js` (`realFinish`), `doc/state-machine-spec.md`, `test/` |
...
| 5.3 | **One staleness contract for the heartbeat.** The bench reads it by **mtime with a 20 s bound**; `console/collect.js`, reached from `bin/spo:1283`, reads it by **content with a 120 s bound**. Two readers, two contracts, one file. | `console/collect.js`, `src/e2e/bench/paths.ts`, `test/` |
