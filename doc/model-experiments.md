...
- **Baseline** (Fable, all PLAN calls up to 2026-09-13,
  ...
  The fable/high `ok` count includes #486's pre-raise deadline kills.
...
- **Watch for:** Opus planning M cards at `high` costing more turns than Fable did at `medium`. The
  cell comparison in `calls` shows it, and it is a reason to try M → medium before reverting the
  model.
