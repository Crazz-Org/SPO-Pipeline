...
with a 20 s bound**; `console/collect.js` — reached from `bin/spo:1153` — reads it **by
...
| **D7** exit-code collapse — the exit code *is* the wire | stale docs — a single-repo instance was found in this very audit (`doc/state-machine-spec.md:157` promises FINISH fast-forwards the main checkout; `realFinish` does not) |
