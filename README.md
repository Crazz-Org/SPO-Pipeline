...
| `console/` | Pipeline console: `render.js` (pure HTML) + `collect.js` (reads journals, queue, accounts, `~/.spo-bench/`, `~/.spo-reports`) + `serve.js`/`system.js`/`usage-scan.js`/`prod-version.js` (the live `--serve` mode: CPU/memory, tokens, starpeace.zz.works) — generated by `bin/spo dashboard`, see below | ✅ v2 (static + live HTML dashboard; `bin/spo` covers the CLI) |
