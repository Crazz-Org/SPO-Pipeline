...
| `bin/spo` | Console CLI (`status` / `task <id>` / `parked` / `resume <id>` read only the journals; `account add` / `account enable`/`disable` / `account clear-cooldown` / `dashboard` write local state; `ask` / `pull` / `intake` / `triage --file` / `recette` write to real GitHub, see `orchestrator/intake.js` and `orchestrator/README.md` § Intake) | ✅ v0 |
