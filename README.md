...
| `bin/spo` | Console CLI (`status` / `task <id>` / `parked` / `nightly` / `resume <id>` / `tokens` / `cost` (deprecated alias for `tokens`) / `accounts` / `reports` read only the journals/registries; `account add` / `account enable`/`disable` / `account clear-cooldown` / `dashboard` write local state; `ask` / `pull` / `pull-reports` / `intake` / `triage --file` / `recette` write to real GitHub, see `orchestrator/intake.js` and `orchestrator/README.md` § Intake; `nightly reprove` asks the bench worker for a manual proof of `main`, human-only, see `doc/manual-nightly-proof.md`) | ✅ v0 |
...
| `console/` | Pipeline console: `render.js` (pure HTML) + `render-deck.js` (the flight deck at `/`) + `collect.js` (reads journals, queue, accounts, `~/.spo-bench/`, `~/.spo-reports`) + `par-times.js` (per-state medians over the whole journal corpus) + `plain-language.js` (the one place a state or a park reason is put into words) + `live-step.js` (what an LLM step is doing while it runs) + `serve.js`/`system.js`/`usage-scan.js`/`usage-rollups.js`/`prod-version.js` (the live `--serve` mode: CPU/memory, tokens + usage trend, starpeace.zz.works) — generated by `bin/spo dashboard`, see below | ✅ v2 (static + live HTML dashboard; `bin/spo` covers the CLI) |
| `journal/` | Per-task append-only event journals — **lives at `~/.spo-state/journal`**, not here (`orchestrator/state-root.js`, `SPO_STATE_DIR`); a release tree is replaced on every deploy, so state inside one is abandoned by the next. This entry stays git-ignored as a guard. | runtime |
| `queue/` | Task queue: one JSON file per task, taken in filename order — **lives at `~/.spo-state/queue`**, same reason as `journal/` above. | runtime |
| `queue-refused/` | Sibling of `queue/` (never a subdirectory of it): queue entries `takeNextTask` refused because their id duplicated an already-terminal (`DONE`/`ABANDONED`) taskDir — **lives at `~/.spo-state/queue-refused`**. No reaper; a duplicate can still arise from a hand-filed queue entry or a crash-recovery re-enqueue race (`intake.js`'s own `taskAlreadyExists` already dedups the `spo pull`/auto-pull paths), so entries are moved aside, never deleted (`orchestrator/README.md` § Task-file format, card #80). | runtime |
...
the units run) · `~/.spo-state/{queue,journal,queue-refused}` (all mutable state) · `~/.claude-accounts` (pool) ·
